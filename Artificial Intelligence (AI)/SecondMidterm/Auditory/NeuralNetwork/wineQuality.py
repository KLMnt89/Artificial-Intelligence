...
def read_dataset():
    ...
    with open("winequality.csv") as f:
        ...
        while True:
            line = f.readline().strip()  # ako ima prazen red
            if line == "":
                break
            parts = line.split(";")
...
def divide_sets(dataset):
    bad_clases = [x for x in dataset if x[-1] == "bad"]
    good_classes = [x for x in dataset if x[-1] == "good"]

    train_set = bad_clases[:int(0.7 * len(bad_clases))] + good_classes[:int(0.7 * len(good_classes))]
    val_set = bad_clases[int(0.7 * len(bad_clases)):int(0.8 * len(bad_clases))] + \
              good_classes[int(0.7 * len(good_classes)):int(0.8 * len(good_classes))]
    test_set = bad_clases[int(0.8 * len(bad_clases)):] + good_classes[int(0.8 * len(good_classes)):]
...
if __name__ == "__main__":
    ...
    classifier1.fit(train_x, train_y)  # treniracko mnozestvo i ocekuvani rezultati
    ...
    for i, c in enumerate([classifier1, classifier2, classifier3]):
        ...
        print(f"Accuracy for classifier {i + 1} is {val_acc}")
    ...
    print(f"Accuracy for final_classifier is {acc / len(test_y)}")
